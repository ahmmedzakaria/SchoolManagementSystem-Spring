...
app.controller('MarksController', ['$scope', '$rootScope', 'StudentInfo', 'StudentRecordBs', 'CommonSupport',
    ...
    function ($scope, $rootScope, StudentInfo, StudentRecordBs, CommonSupport, StudentSession,
        ...
        ob.generateMarkList = function (studentRecordBsList) {
            console.log(ob.studentRecordBsList);
            ob.markList=[];
            for (var i = 0; i < studentRecordBsList.length; i++) {
                var marks = {};
                marks.studentRecordBs = studentRecordBsList[i];
                ob.markList.push(marks);

            }
            console.log(ob.markList);

        };
        ob.addMarks = function () {
            console.log('Inside save');
            for (var i = 0; i < ob.markList.length; i++) {
                ob.markList[i].subjects=ob.subjects;
                ob.markList[i].exams=ob.exams;

            }
            ob.studentInfo=new StudentInfo();
            ob.studentInfo.markList =ob.markList;
...
//        console.log("rbsId: "+rbsid);
        ob.getAttendanceForStudent=function(){
            ...
            ob.studentInfo = StudentInfo.get({path: 'attendance', userId: rbsid}, function () {
                console.log(ob.studentInfo);
                ob.markList=ob.studentInfo.attendancesList;
